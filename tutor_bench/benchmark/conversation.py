...
Provides both a synchronous rollout loop and a batched rollout loop. A
Transcript records the turns generated by the tutor-under-test and the
...
from tutor_bench.benchmark.scenarios import Scenario, _build_reference_transcript
from tutor_bench.benchmark.student import build_student_system_prompt, get_or_generate_trait, resolve_student
...
class Transcript:
    ...
    tutor_usage: dict = field(default_factory=lambda: {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0})
    student_usage: dict = field(default_factory=lambda: {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0})
...
def _parse_tutor_tokens(text: str) -> tuple[str, bool, bool]:
    ...
    cleaned = text.replace(END_TOKEN, "").replace(PROBLEM_CHANGE_TOKEN, "").replace(NEXT_PROBLEM_TOKEN, "").rstrip()
    if has_end:
    ...
    *,
    ...
    while speaking_turns < max_turns:
        ...
        if not messages and not (ended or problem_change):
            messages = ["..."]
        if messages:
            extra, next_turn_num = _append_turns_to_extra(transcript, messages, "TUTOR", extra, next_turn_num)
        ...
        extra, next_turn_num = _append_turns_to_extra(transcript, messages, "STUDENT", extra, next_turn_num)
...
def run_conversations_batch(
    ...
    *,
    ...
    via run_batch, then all active student calls go in the next batch. A
    scenario that emits [END] or [PROBLEM_CHANGE] is pruned from later rounds.
    Latencies are omitted in batch mode (not available from the batch API).
    ...
    from tutor_bench.benchmark.client import build_batch_entry, run_batch
    ...
    for round_num in range(math.ceil(max_turns / 2)):
        ...
        # --- Tutor batch ---
        logger.info("Round %d - tutor batch (%d scenarios)", round_num + 1, len(active_ids))
        tutor_entries = []
        ...
        logger.info("Round %d - student batch (%d scenarios)", round_num + 1, len(active_ids))
        student_entries = []
        for sid in active_ids:
            scenario = scenario_map[sid]
            head, tail = _build_role_prompt(
