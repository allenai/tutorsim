"""Scenario dataclass and build_scenarios."""
...
def _missing_dataset_message(set_name: str, path: Path) -> str:
    return (
        ...
        "Create it with the `dataset build ...` command or install/download the "
...
def validate_dataset(set_name: str, root: str = "scenarios") -> dict:
    ...
    if manifest is None:
        raise FileNotFoundError(f"Dataset manifest not found: {manifest_path(set_name, root)}")

    expected_name = manifest.get("name")
    if expected_name and expected_name != set_name:
        raise ValueError(f"Dataset manifest name mismatch: expected {set_name!r}, got {expected_name!r}")

    schema_version = manifest.get("schema_version")
    if schema_version != DATASET_SCHEMA_VERSION:
        raise ValueError(f"Unsupported dataset schema_version {schema_version!r}; expected {DATASET_SCHEMA_VERSION}")
    ...
    if expected_count is not None and int(expected_count) != len(scenarios):
        raise ValueError(f"Dataset count mismatch: manifest={expected_count}, loaded={len(scenarios)}")
    ...
    if expected_hash and expected_hash != content_hash:
        raise ValueError(f"Dataset content hash mismatch: manifest={expected_hash}, actual={content_hash}")
...
# ---------------------------------------------------------------------------
# Local-only data loaders. Read from explicit directories; no remote storage.
# ---------------------------------------------------------------------------
...
def _transform_normalized_record(rec: dict) -> dict:
    """Transform a normalized JSONL record into the internal transcript format.

    Normalized format: turns[] (dialogue only) + enrichments[] (non-dialogue,
    with before_turn). Enrichments are inserted before their associated dialogue
    turn with is_enrichment=True so they appear in excerpts as context but don't
    shift turn numbering.
    """
...
def _load_jsonl_index(path: str) -> dict[str, dict]:
    ...
    Local-only variant: no remote backend, no caching across calls.
    Returns {conv_id: conversation_dict}.
    """
...
def _resolve_cluster(members: list[dict], conversation: dict, turn_start: int, turn_end: int) -> "dict | None":
...
def _prefix_turns_to_context(conversation: dict, cut_turn: int) -> list[dict]:
    """Convert prefix turns into the Scenario context format: [{turn_number, role, text}].
...
def _main() -> None:
    parser = argparse.ArgumentParser(description="scenario tools")
    ...
    build_p.add_argument(
        "--step-up-jsonl", dest="step_up_jsonl", default=None, help="Path to normalized step_up JSONL transcript file"
    )
