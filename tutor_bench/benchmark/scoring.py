"""Judgment dataclass and synthetic-conversation builder for benchmark scoring.

Implements the three-pass LLM-judge scoring pipeline that runs over a single
synthetic scenario + transcript:

  - _format_excerpt: render a transcript excerpt with detected-moment markers
    (the benchmark always uses context_window=0, i.e. no before/after context).
  - _suggestion_text: map a situation label to the suggestion sentence injected
    into the scorer prompt.
  - _build_annotate_entries / _parse_and_merge: build the Pass 1 batch entries
    and parse/merge the model results back into per-moment records.
...
# ---------------------------------------------------------------------------
# Scoring pass 1: excerpt builder
...
# ---------------------------------------------------------------------------
# Scoring pass 1: suggestion text
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Scoring pass 1: build batch entries
...
def _parse_and_merge(
    ...
    """Parse batch results and merge them with detections into final records.
    ...
    - Accumulates usage from pass 1 (detections_by_conv) + parsed results.
...
class Judgment:
    ...
    Fields carry the scored situation/action/result plus identifiers for the
    scenario that generated the conversation being scored.
    """
...
def _build_synthetic_conversation(
    ...
    Returns:
        ...
        scenario.id (the scenario_id, not the raw conv_id).
    """
    ...
    # raw conv_id. The conversation is keyed by scenario.id directly.
    conv = {
    ...
    # The benchmark always scores the scaffolding lens for detected moments.
    ann_type = "scaffolding"
...
# ---------------------------------------------------------------------------
# Scoring pass 2: decompose
#
# Only the benchmark path is implemented:
...
# ---------------------------------------------------------------------------
# Scoring pass 3: structure
#
# Only the benchmark/in-memory path is implemented:
...
def _build_structure_entries(
    ...
    batch used by the structure pass.
...
# ---------------------------------------------------------------------------
# score(scenario, transcript) -> Judgment
#
# Orchestrates the three-pass scoring pipeline over a single synthetic scenario:
...
#
# Scorer model/params come from config.scorer_spec().
# context_window=0 always (benchmark-only override; see annotate pass docstring).
# Usage accumulated across all 3 passes is attached as Judgment.usage.
# ---------------------------------------------------------------------------


def score(scenario: Any, transcript: Any) -> "Judgment":
    """Run the three-pass scorer over one scenario+transcript and return a Judgment.
    ...
    Scorer model: read from config.scorer_spec().
    context_window=0: forced for the benchmark path (no pre-cut context leaks).
    Usage is summed across all 3 passes and stored in the returned Judgment.usage.
    ...
    Returns:
        Judgment with all fields populated.
    """
    ...
    if not detections_by_id:
        # No generated turns: return a minimal Judgment with empty fields.
        return Judgment(
    ...
    # All three sub-passes are combined into a single run_batch call, then split
    ...
    for (conv_id, idx), entry in zip(action_decomp_locs, action_decomp_entries, strict=False):
        key = entry["key"]
    ...
    for (conv_id, idx), entry in zip(result_decomp_locs, result_decomp_entries, strict=False):
        key = entry["key"]
        raw = decompose_raw.get(key, {})
        text = raw.get("text", "")
        try:
    ...
    for (conv_id, idx), entry in zip(overscaffold_locs, overscaffold_entries, strict=False):
        key = entry["key"]
        raw = decompose_raw.get(key, {})
        text = raw.get("text", "")
        try:
    ...
    for _conv_id, conv_data in results.items():
