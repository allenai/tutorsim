...
# Provider routing by model-name prefix; order matters (first match wins).
PROVIDER_PREFIXES = [
    ("gemini", "gemini"),
    ("gpt", "openai"),
    ("o1", "openai"),
...
# Model-name prefixes known to support vision (image) input.
...
def infer_provider(model: str) -> str:
    ...
    for prefix, provider in PROVIDER_PREFIXES:
        if model_lower.startswith(prefix):
            return provider
    raise ValueError(
        f"Cannot infer provider for model '{model}'. Expected prefix: {', '.join(p for p, _ in PROVIDER_PREFIXES)}"
    )
...
class ModelClient:
    ...
    def generate(
        ...
        *,
        ...
        from tutor_bench.benchmark.config import get_retry_config
            ...
            try:
                if self.provider == "gemini":
                    ...
                    resp = self._generate_together(
                        prompt, json_mode, max_tokens, timeout, cacheable_prefix=cacheable_prefix
                    )
                else:
                ...
                if attempt < max_retries - 1:
                    logger.warning(
                        "API error (attempt %d/%d): %s. Retrying in %ds...", attempt + 1, max_retries, e, delay
                    )
                    time.sleep(delay)
                else:
    ...
    def _generate_gemini(
        ...
        # No explicit Gemini cache wiring yet; concatenate the cacheable head
        # into the prompt so behavior is semantically correct even without a
        # real cache hit.
    ...
    def _generate_together(self, prompt, json_mode, max_tokens, timeout, cacheable_prefix: str | None = None):
...
# Per-model max output token caps (from the model catalog). Requests above the
# cap are rejected by the API. Keys are matched by startswith().
...
class _LocalBackend:
    """Minimal local-file storage backend."""
    ...
    def get_presigned_url(self, path: str, expires_seconds: int = 172800) -> str:
        raise NotImplementedError(
            "get_presigned_url is only available with a remote backend. Local backend has no pre-signed URL capability."
        )
...
def _get_backend() -> "_LocalBackend":
    """Return the active storage backend (the LocalBackend singleton)."""
...
def _should_use_presigned_url() -> bool:
    """True when the storage backend supports pre-signed URLs.

    The local backend never does, so this returns False.
    """
...
def _presigned_url(rel_path: str, expires_seconds: int = 172800) -> str:
    ...
    The local backend has no pre-signed URLs and raises NotImplementedError.
    """
...
def _build_image_blocks_gemini(image_paths: list[str]) -> list[dict]:
    ...
    Gemini does not accept remote URIs; always inline.
    """
    blocks = []
    for path in image_paths:
        blocks.append(
...
def run_sync_entries(client: "ModelClient", entries: list[dict], json_mode: bool = True, max_tokens: int = 0) -> dict:
...
def run_batch(
    ...
    if existing_batch_id:
        logger.info("Resuming in-flight %s batch %s (%d entries)", provider, existing_batch_id, len(entries))
    else:
        logger.info("Running batch (%s): %d entries, display_name=%s", provider, len(entries), display_name)
...
def _run_batch_gemini(
    ...
    from tutor_bench.benchmark.config import get_batch_timeout
    ...
    if existing_batch_id:
        ...
        uploaded_file = gemini_client.files.upload(
            file=jsonl_path, config=types.UploadFileConfig(display_name=display_name, mime_type="jsonl")
        )
    ...
    try:
        ...
        while batch_job.state.name not in completed_states:
            if time.monotonic() - poll_start > batch_timeout:
                raise RuntimeError(f"Gemini batch timed out after {batch_timeout}s (state: {batch_job.state.name})")
...
def _run_batch_openai(
    ...
    try:
        ...
        while batch_job.status not in terminal_states:
            if time.monotonic() - poll_start > batch_timeout:
                raise RuntimeError(f"OpenAI batch timed out after {batch_timeout}s (state: {batch_job.status})")
...
def _run_batch_anthropic(
    ...
    from tutor_bench.benchmark.config import get_batch_timeout, get_retry_config
    ...
    if existing_batch_id:
        ...
        thinking_param = _anthropic_thinking_param(client.model, thinking_budget) if thinking else None
        thinking_min = 0
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    delay = base_delay * (2**attempt)
                    logger.warning(
                        "Batch submit error (attempt %d/%d): %s. Retrying in %ds...", attempt + 1, max_retries, e, delay
                    )
                    time.sleep(delay)
                else:
                    raise
    ...
    while message_batch.processing_status != "ended":
        if time.monotonic() - poll_start > batch_timeout:
            raise RuntimeError(
                f"Anthropic batch timed out after {batch_timeout}s (state: {message_batch.processing_status})"
            )
