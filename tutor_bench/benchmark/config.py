"""Config loading and tutor/student registries."""
...
_CONFIG_ENV_VAR = "TUTOR_BENCH_CONFIG"
...
def describe_config_source(path: str | os.PathLike | None = None) -> str:
    ...
    if kind == "package":
        return f"tutor-bench:{ident}"
    return str(Path(ident))


def load_config(path: str | os.PathLike | None = None) -> dict:
    """Load and parse a config file, with module-level caching.

    Args:
        path: Optional explicit config path. If omitted, precedence is
            TUTOR_BENCH_CONFIG, cwd/config.yaml, then packaged default_config.yaml.
...
def get_registered_tutor(name: str) -> Callable | None:
...
def get_registered_student(name: str) -> Callable | None:
...
def resolve_model(model_id: str, config_path: str | os.PathLike | None = None) -> dict:
    ...
    if model_id not in cfg["models"]:
        ...
        raise ValueError(f"Model '{model_id}' not in roster. Valid models: {', '.join(valid_ids)}")
