"""Student trait generation and oracle simulator prompt.
...
class TraitGenerator:
    ...
    def _generate_joined_trait(
        ...
        temperature: float = 0.7,
        max_tokens: int | None = None,
        individual_dimensions: list | None = None,
    ) -> str:
...
_DEFAULT_CACHE_DIR = "results/benchmark/_trait_cache"
...
def _trait_cache_filename(scenario, trait_mode: str) -> str:
    ...
    Key: (conv_id, cut_turn, trait_mode).
    """
...
def get_or_generate_trait(
    ...
    Args:
        ...
        cache_dir: Directory for cache files (default: results/benchmark/_trait_cache/).
...
# ---------------------------------------------------------------------------
# Oracle student system prompt builder
# ---------------------------------------------------------------------------
...
def _get_shared_generation_instructions() -> str:
    """Return the shared generation instructions text (oracle mode)."""
    return (
...
def resolve_student(student_id: str | None = None) -> dict:
    ...
    """
    from tutor_bench.benchmark.client import ModelClient
    from tutor_bench.benchmark.config import get_registered_student, student_spec
