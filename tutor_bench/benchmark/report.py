"""Per-set aggregate rate math for benchmark results.
...
Fields read off the scoring objects:
  scenario.dimension  (= scenario.rubric["gold"], set at build time)
  judgment.action_label  (first and only Judgment per scenario)
  judgment.result_label  (str "pos" | "neg" | "unclear" | "no_evidence")
  judgment.overscaffold_decomposed  (non-empty list means over-scaffold detected)
...
def aggregate(scenarios: list, judgments: list) -> dict:
    ...
    Args:
        scenarios: list of Scenario objects (canonical order).
        judgments: list of Judgment objects, parallel to scenarios.
    ...
    overscaffold.rate     = scenarios with non-empty overscaffold_decomposed
                            ...
                            (not applicable for the Judgment dataclass --
    ...
    for scenario, judgment in zip(scenarios, judgments, strict=False):
        ...
        result_label = judgment.result_label
        ...
        # The Judgment dataclass always has overscaffold_decomposed, so
        # any_overscaffold_field becomes True as soon as we process any scenario
        # (available=True whenever n_total > 0).
        any_overscaffold_field = True  # field always present on Judgment dataclass
        if has_over:
            over_yes += 1
    ...
    return {
        ...
        # recomputed from the same data without re-running scoring.
        "scaffold_calibrated": {
