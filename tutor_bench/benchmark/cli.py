"""CLI entry point for tutor-bench benchmark runs.
...
Provides:
  ...
  main() / argparse "run" subcommand:
    tutor-bench run --tutors X [--modes ...] [--dataset ...] [--sample N]
...
# Heavy modules are imported at module level (not inside SDK-touching code) and
# re-exported here so test patch targets such as
# `tutor_bench.benchmark.cli.conversation.run_conversation` resolve correctly.
import tutor_bench.benchmark.conversation as conversation
import tutor_bench.benchmark.report as report
import tutor_bench.benchmark.results as results
import tutor_bench.benchmark.scoring as scoring
from tutor_bench.benchmark.config import build_run_config
from tutor_bench.benchmark.scenarios import (
...
def expand_cells(tutors: list[str], modes: list[str]) -> list[dict]:
    ...
    from tutor_bench.benchmark.client import infer_provider
    from tutor_bench.benchmark.config import get_registered_tutor
...
def _resource_hashes(root: str) -> dict[str, str]:
    ...
    base = files("tutor_bench.benchmark") / root
    out: dict[str, str] = {}
...
def run_cell(
    ...
    *,
    ...
    Sequence:
      ...
      4. aggregate over completed judgments -> write_summary
      5. Return run_id
    ...
    def _run_trial(trial_idx: int) -> tuple:
            ...
            if results.is_done(run_id, resume_sid, results_root=results_root):
                ...
                if score_dict is not None:
                    ...
                    try:
                        ...
                        completed_judgments.append(ann)
                        counts["succeeded"] += 1
                        counts["resumed"] += 1
        ...
        if completed_scenarios:
            metrics = report.aggregate(completed_scenarios, completed_judgments)
        else:
            raise RuntimeError(
    ...
    # Merge latency + token blocks into summary
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="tutor-bench",
        description="tutor-bench benchmark runner",
    )
    ...
    run_p.add_argument(
        ...
        default=None,
        metavar="FILE",
        help="Config file override (default: TUTOR_BENCH_CONFIG, ./config.yaml, then packaged default)",
    )
    run_p.add_argument(
        "--modes",
        nargs="+",
...
def main(argv=None) -> None:
    ...
    if args.command == "run":
        if args.config:
            os.environ["TUTOR_BENCH_CONFIG"] = args.config
