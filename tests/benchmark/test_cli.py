"""Tests for tutor_bench.benchmark.cli -- run_cell end-to-end.
...
def _make_judgment(sid: str) -> Judgment:
    """Build a minimal Judgment for testing."""
    return Judgment(
        scenario_id=sid,
...
_CONV_PATCH = "tutor_bench.benchmark.cli.conversation.run_conversation"
_SCORE_PATCH = "tutor_bench.benchmark.cli.scoring.score"
_LOAD_PATCH = "tutor_bench.benchmark.cli.load_scenarios"
_CFG_PATCH = "tutor_bench.benchmark.cli.build_run_config"
...
def test_run_cell_writes_all_files(tmp_path):
    ...
    import tutor_bench.benchmark.report as report_mod
    from tutor_bench.benchmark.cli import run_cell

    scenarios = list(FIXTURE_SCENARIOS)
    transcripts = [_make_transcript(s.id) for s in scenarios]
    judgments = [_make_judgment(s.id) for s in scenarios]
    expected_summary = report_mod.aggregate(scenarios, judgments)
    ...
    with (
        ...
        patch(_SCORE_PATCH, side_effect=judgments),
    ):
        run_id = run_cell(
    ...
    # summary metrics == aggregate of the 2 judgments
...
def test_run_cell_resumes(tmp_path):
    ...
    with (
        ...
        patch(_SCORE_PATCH, side_effect=list(judgments)),
    ):
        run_id = run_cell(
            tutor="claude-opus-4-8",
            mode="plain",
...
def test_run_cell_skips_on_score_error(tmp_path):
    ...
    good_judgment = _make_judgment(good_scenario.id)
...
def test_scheduler_within_lane_sequential(tmp_path):
    ...
    from tutor_bench.benchmark.cli import expand_cells, run_sweep
...
def test_trials_1_summary_matches_single_run(tmp_path):
    """trials=1: summary.json must be byte-compatible with the single-run format
...
def test_report_writes_leaderboard_md_and_csv(tmp_path):
    ...
    csv_lines = [line for line in csv_text.strip().splitlines() if line.strip()]
...
def test_main_run_smoke(tmp_path):
    ...
    judgment = _make_judgment("smoke_001")
    ...
    with (
        ...
        patch(_SCORE_PATCH, return_value=judgment),
        patch("tutor_bench.benchmark.cli.run_sweep") as mock_sweep,
    ):
...
def test_main_run_missing_dataset_exits_cleanly(capsys):
    ...
    from tutor_bench.benchmark.cli import main
    from tutor_bench.benchmark.scenarios import DatasetNotFoundError
...
def test_run_config_argument_becomes_active_config_for_late_resolution(tmp_path, monkeypatch):
    """`tutor-bench run --config FILE` also affects later no-arg config lookups."""
    from tutor_bench.benchmark.cli import main
    from tutor_bench.benchmark.config import _reset_config_cache
...
""".strip(),
    ...
    def fake_run_sweep(cells, run_cfg, *, date, results_root, trait_cache_dir):
        from tutor_bench.benchmark.config import resolve_model, scorer_spec, student_spec
    ...
    monkeypatch.setattr("tutor_bench.benchmark.cli.run_sweep", fake_run_sweep)
    original_config = os.environ.pop("TUTOR_BENCH_CONFIG", None)
    _reset_config_cache()
    ...
    try:
        ...
        assert os.environ["TUTOR_BENCH_CONFIG"] == str(config_path)
    finally:
        if original_config is None:
            os.environ.pop("TUTOR_BENCH_CONFIG", None)
        else:
            os.environ["TUTOR_BENCH_CONFIG"] = original_config
        _reset_config_cache()
