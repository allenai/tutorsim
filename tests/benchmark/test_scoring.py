...
from tutor_bench.benchmark.scoring import (
    DEFAULT_ACTION_LABEL,
    DEFAULT_RESULT_LABEL,
    Judgment,
    _build_annotate_entries,
    _build_decompose_entries,
...
# ---------------------------------------------------------------------------
# Judgment dataclass + _build_synthetic_conversation
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Annotate pass — excerpt builder, suggestion text, build entries, parse
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Decompose pass — _coerce_facets, _build_decompose_entries, junk skip
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Structure pass -- _parse_action_label, _parse_result_label,
...
# ---------------------------------------------------------------------------
# score(scenario, transcript) -> Judgment (3-pass end-to-end)
# ---------------------------------------------------------------------------
...
def _patch_score(fake_run_batch, fake_client=None):
    ...
    in the client module, which is where score() resolves them via local import.

    score() does:
        from .client import ModelClient, run_batch
    so the names to patch are tutor_bench.benchmark.client.run_batch and
    tutor_bench.benchmark.client.ModelClient.
    """
...
def test_score_returns_annotation(fixture_scenario, fixture_transcript):
    """score() with mocked run_batch returns a populated Judgment."""
...
def test_score_accumulates_usage_across_passes(fixture_scenario, fixture_transcript):
    """score() must accumulate usage from all 3 passes into Judgment.usage."""
    ...
    assert hasattr(result, "usage"), "Judgment must have a 'usage' attribute"
    usage = result.usage
