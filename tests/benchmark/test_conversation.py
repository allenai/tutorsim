...
def _patch_all(monkeypatch, tutor_responses, student_responses, *, trait_persona="fake-persona"):
    ...
    monkeypatch.setattr(
        "tutor_bench.benchmark.conversation.resolve_tutor",
        lambda id: {"kind": "hosted", "client": tutor_client, "kwargs": {}},
    )
    monkeypatch.setattr(
        "tutor_bench.benchmark.conversation.resolve_student",
        lambda id=None: {"kind": "hosted", "client": student_client, "kwargs": {}},
    )
    monkeypatch.setattr(
        "tutor_bench.benchmark.conversation.build_tutor_system_prompt",
        lambda mode, **kw: "TUTOR_SYS",
    )
    monkeypatch.setattr(
        "tutor_bench.benchmark.conversation.build_student_system_prompt",
        lambda **kw: "STUDENT_SYS",
    )
    monkeypatch.setattr(
        "tutor_bench.benchmark.conversation.get_or_generate_trait",
...
def test_registered_tutor(monkeypatch):
    ...
    from tutor_bench.benchmark.conversation import (
        run_conversation,
    )
    ...
    monkeypatch.setattr("tutor_bench.benchmark.conversation.build_tutor_system_prompt", lambda mode, **kw: "SYS")
    monkeypatch.setattr("tutor_bench.benchmark.conversation.build_student_system_prompt", lambda **kw: "SYS")
    monkeypatch.setattr("tutor_bench.benchmark.conversation.get_or_generate_trait", lambda *a, **kw: "persona")
...
def test_cacheable_prefix_passed_to_client(monkeypatch):
    ...
    monkeypatch.setattr("tutor_bench.benchmark.conversation.build_tutor_system_prompt", lambda mode, **kw: "TUTOR_SYS")
    monkeypatch.setattr("tutor_bench.benchmark.conversation.build_student_system_prompt", lambda **kw: "STUDENT_SYS")
    monkeypatch.setattr("tutor_bench.benchmark.conversation.get_or_generate_trait", lambda *a, **kw: "fake-persona")
...
def _patch_batch(monkeypatch, tutor_batch_results: list[dict], student_batch_results: list[dict]):
    """Patch resolve_tutor, resolve_student, system prompts, trait, and client.run_batch.
    ...
    interleaved = []
    for t_res, s_res in zip(tutor_batch_results, student_batch_results, strict=False):
    ...
    # Patch at client module since run_conversations_batch imports lazily from there.
    monkeypatch.setattr("tutor_bench.benchmark.client.run_batch", run_batch_mock)
    monkeypatch.setattr(
        "tutor_bench.benchmark.client.build_batch_entry",
