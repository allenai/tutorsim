...
# Repo root relative to this file: tests/benchmark/ -> ../../
...
class TestExactCounts:
    ...
    def test_dimension_split(self, raw_scenarios):
        ...
        assert counts["scaffolding"] == 260, f"Expected 260 scaffolding, got {counts['scaffolding']}"
    ...
    def test_all_have_rubric_gold(self, raw_scenarios):
        missing = [s["id"] for s in raw_scenarios if not s.get("rubric", {}).get("gold")]
    ...
    def test_reference_counts(self, raw_scenarios):
        ...
        non_empty = sum(1 for s in raw_scenarios if s.get("student", {}).get("reference", ""))
...
class TestOrderMatchesIdList:
    def test_order_matches_id_list(self, raw_scenarios, canonical_ids):
        ...
        expected_ordered = [
            f"balanced_520:{raw_id}" for raw_id in canonical_ids if f"balanced_520:{raw_id}" in actual_set
        ]

        assert actual_ids == expected_ordered, "Scenario order does not match canonical id-list order"
...
class TestLoadScenariosRoundTrip:
    ...
    def test_load_scenarios_first_last_ids(self, canonical_ids):
        ...
        assert scenarios[0].id == expected_first, f"First id mismatch: {scenarios[0].id} != {expected_first}"
        assert scenarios[-1].id == expected_last, f"Last id mismatch: {scenarios[-1].id} != {expected_last}"
