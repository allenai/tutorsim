...
from tutor_bench.benchmark.client import (
...
# ===================================================================
# generate() + provider builders + retry + usage/latency
# ===================================================================
...
def test_generate_retries_then_succeeds(monkeypatch):
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-test")
    with patch("anthropic.Anthropic") as MockAnthropic, patch("tutor_bench.benchmark.client.time.sleep"):
        client_obj = MagicMock()
...
# ===================================================================
# vision image-block builders + cacheable-prefix
# ===================================================================
...
class TestShouldUsePresignedUrl:
    def test_returns_false_by_default(self):
        # Local storage only -> always False.
...
class TestBase64Bytes:
    def test_returns_base64_string_from_file(self, tmp_path):
        ...
        with patch("tutor_bench.benchmark.client._get_backend") as mock_be:
...
class TestPresignedUrl:
    def test_delegates_to_backend(self):
        ...
        mock_be.return_value.get_presigned_url.assert_called_once_with("images/img.png", expires_seconds=3600)
...
class TestBuildImageBlocksAnthropic:
    ...
    def test_base64_block_shape(self):
        with patch("tutor_bench.benchmark.client._base64_bytes", return_value="ZmFrZQ=="):
    ...
    def test_url_block_shape(self):
        with patch("tutor_bench.benchmark.client._presigned_url", return_value="https://s3.example.com/img.png"):
    ...
    def test_multiple_images(self):
        with patch("tutor_bench.benchmark.client._base64_bytes", side_effect=["b64a", "b64b"]):
...
class TestBuildImageBlocksOpenAI:
    ...
    def test_presigned_url_shape(self):
        with patch("tutor_bench.benchmark.client._presigned_url", return_value="https://cdn.example.com/img.jpg"):
            blocks = _build_image_blocks_openai(["photo.jpg"], use_url=True)
        b = blocks[0]
        assert b["image_url"]["url"] == "https://cdn.example.com/img.jpg"

    def test_multiple_images(self):
        with patch("tutor_bench.benchmark.client._base64_bytes", side_effect=["b64x", "b64y"]):
...
class TestBuildImageBlocksGemini:
    ...
    def test_multiple_images(self):
        with patch("tutor_bench.benchmark.client._base64_bytes", side_effect=["a", "b"]):
...
# ===================================================================
# run_sync_entries + run_batch (anthropic/openai/gemini)
# ===================================================================
...
def test_run_sync_entries_collects_by_key(monkeypatch):
    ...
    with patch("anthropic.Anthropic") as MockAnthropic:
        ...
        with patch.object(
            c,
            "generate",
            return_value=ModelResponse("R", {"input_tokens": 1, "output_tokens": 1, "total_tokens": 2}, 0.0),
        ):
            out = run_sync_entries(c, [build_batch_entry("k1", "p1"), build_batch_entry("k2", "p2")])
...
def test_run_batch_openai_parses_results(monkeypatch):
    ...
    with patch("openai.OpenAI") as MockOpenAI, patch("tutor_bench.benchmark.client.time.sleep"):
        client_obj = MagicMock()
        uploaded = MagicMock()
        uploaded.id = "file-1"
...
def test_run_batch_gemini_parses_results(monkeypatch):
    ...
    with patch("google.genai.Client") as MockGenai, patch("tutor_bench.benchmark.client.time.sleep"):
        client_obj = MagicMock()
        uploaded = MagicMock()
