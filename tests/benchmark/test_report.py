...
Fixture: 6 (Scenario, Judgment) pairs, covering all branches:
...
Expected values computed BY HAND from the scoring formulas:
...
from tutor_bench.benchmark.report import aggregate, leaderboard, leaderboard_row, view
...
def _make_judgment(
    ...
    """Minimal Judgment stub: only .action_label, .result_label,
...
def test_aggregate_golden(six_pairs):
    ...
    scenarios, judgments = six_pairs
    result = aggregate(scenarios, judgments)
...
def test_aggregate_neither_gold_excluded():
    ...
    judgments = [_make_judgment("scaffolding", "no_evidence", [])]
    result = aggregate(scenarios, judgments)
...
def test_aggregate_result_label_as_list():
    """result_label as list: 'pos' in list triggers outcome_pos."""
...
def test_leaderboard_sorted_desc_by_scaffold_cal():
    ...
    lines = [ln for ln in md.split("\n") if "|" in ln and "---" not in ln and "tutor_model" not in ln]
...
def test_leaderboard_none_formatting():
    ...
    csv_lines = [ln for ln in csv_str.split("\n") if "model-gamma" in ln]
