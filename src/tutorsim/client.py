...
Behavior ported verbatim from annotator/core/client.py.
"""
import base64
import json
import logging
...
# Ported from annotator/core/client.py:41-56 — order matters (first match wins).
PROVIDER_PREFIXES = [
    ("gemini", "gemini"),
    ("gpt", "openai"),
    ("o1", "openai"),
...
# Ported verbatim from annotator/core/client.py:71-85.
...
def validate_vision_support(model: str) -> None:
    ...
    Ported verbatim from annotator/core/client.py:78-85.
    """
    m = model.lower()
...
# Ported from annotator/core/client.py:125-133.
...
class ModelClient:
    ...
    def generate(self, prompt: str,
        ...
        Ported verbatim from annotator/core/client.py:337-398.
        """
    ...
    def _generate_gemini(self, prompt, json_mode, max_tokens, timeout,
        ...
        Ported verbatim from annotator/core/client.py:400-445.
        """
        config = {
    ...
    def _generate_openai(self, prompt, json_mode, max_tokens, timeout,
        ...
        Ported verbatim from annotator/core/client.py:447-488.
        """
        if images:
    ...
    def _generate_together(self, prompt, json_mode, max_tokens, timeout,
        ...
        Ported verbatim from annotator/core/client.py:490-521.
        """
        content = (cacheable_prefix or "") + prompt
        kwargs = {
            "model": self.model,
    ...
    def _generate_anthropic(self, prompt, json_mode, max_tokens, timeout,
        ...
        Ported verbatim from annotator/core/client.py:523-605.
        """
        system_parts = []
        if json_mode:
            system_parts.append(
...
# Ported from annotator/core/client.py:61-66 — Provider-specific max output token limits
MAX_OUTPUT_TOKENS = {
    "gemini": 65536,
    "openai": 128000,
    "anthropic": 128000,
...
# Ported from annotator/core/client.py:137-140 — Per-model max output token caps (from the model catalog).
...
# Ported from annotator/core/client.py:88-93 — MIME type mapping by file extension
_MIME_BY_EXT = {
    ".jpg": "image/jpeg",
    ".jpeg": "image/jpeg",
    ".png": "image/png",
...
def _mime_from_path(path: str) -> str:
    ...
    Ported from annotator/core/client.py:96-100.
    """
...
def _strip_json_fences(text: str) -> str:
    ...
    Ported from annotator/core/client.py:623-632.
    """
    stripped = text.strip()
...
def _extract_anthropic_text(content) -> str:
    ...
    Ported verbatim from annotator/core/client.py:611-620.
    """
...
def _interleave_text_and_images(
    ...
    Ported verbatim from annotator/core/client.py:213-256.
    """
    parts: list[dict] = []
    cursor = 0
    used: set[int] = set()
...
def _should_use_presigned_url() -> bool:
    ...
    Ported verbatim from annotator/core/client.py:103-107.
...
def _base64_bytes(rel_path: str) -> str:
    ...
    Ported verbatim from annotator/core/client.py:110-113.
    """
...
def _presigned_url(rel_path: str, expires_seconds: int = 172800) -> str:
    ...
    Ported verbatim from annotator/core/client.py:116-118.
...
def _build_image_blocks_anthropic(
    ...
    Ported verbatim from annotator/core/client.py:156-174.
    """
    blocks = []
...
def _build_image_blocks_openai(
    ...
    Ported verbatim from annotator/core/client.py:177-188.
    """
    blocks = []
    for path in image_paths:
        if use_url:
...
def _build_image_blocks_gemini(image_paths: list[str]) -> list[dict]:
    ...
    Ported verbatim from annotator/core/client.py:191-201.
    """
    blocks = []
    for path in image_paths:
        blocks.append({
...
def _extract_entry(entry: dict) -> tuple[str, str, bool, int, list[str]]:
    ...
    Ported verbatim from annotator/core/client.py:635-644.
    """
    key = entry["key"]
...
def run_sync_entries(client: 'ModelClient', entries: list[dict],
    ...
    Ported verbatim from annotator/core/client.py:695-727.
    """
    raw_entries = {}
    total = len(entries)
...
def run_batch(client: 'ModelClient', entries: list[dict],
    ...
    Ported verbatim from annotator/core/client.py:734-776.
    """
...
def _run_batch_gemini(client, entries, json_mode, display_name, poll_interval,
    ...
    Ported verbatim from annotator/core/client.py:783-904.
    """
    import tempfile
...
def _run_batch_openai(client, entries, json_mode, display_name, poll_interval,
    ...
    Ported verbatim from annotator/core/client.py:911-1042.
    """
    import tempfile
    from tutorsim.config import get_batch_timeout
...
def _run_batch_anthropic(client, entries, json_mode, display_name, poll_interval,
    ...
    Ported verbatim from annotator/core/client.py:1049-1198.
    """
