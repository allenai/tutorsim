...
This file documents historical/private raw data used during earlier research
iterations. It is not the setup guide for the packaged `tutorsim` runtime. Some
scripts in this directory still refer to the old `annotator` package and should
be treated as legacy until they are ported or removed.

For the supported dataset build path, see the root `README.md` and
